...
> **Naming note — do not confuse this with the actual playbook.** Despite the similar name, this file is **not** the canonical [TFRS Engineering Playbook](https://github.com/TFRS-Admin/tfrs-engineering-playbook) (`tfrs-engineering-playbook`). That external repository, plus this repository's baseline files ([`AGENTS.md`](../AGENTS.md), [`CLAUDE.md`](../CLAUDE.md), [`AI_AGENT_OPERATING_MODEL.md`](../AI_AGENT_OPERATING_MODEL.md), [`DECISION_ROUTER.md`](../DECISION_ROUTER.md)), is the actual workflow authority for this repository. **Start at [`AGENTS.md`](../AGENTS.md), always** — not here.
>
> **Superseded as the constitution; retained as the canonical local historical design record.** As of the TFRS Engineering Playbook adoption sprint (see [`docs/PLAYBOOK_ADOPTION.md`](./PLAYBOOK_ADOPTION.md)), this file's proposed command vocabulary (`/spec /plan /build /test /review`), `PROJECT_STATE.md`/`IDEA_BACKLOG.md` file scheme, and self-declared "governing document" status below are **not** in effect — the adopted playbook's lifecycle (`commands/review.md` → `roadmap.md` → `plan.md` → `backlog.md` → `execute.md` → `verify.md` → `ship.md`, run live against `tfrs-engineering-playbook`) governs instead. Do not treat anything below this notice as authoritative for how work moves through this system.
>
