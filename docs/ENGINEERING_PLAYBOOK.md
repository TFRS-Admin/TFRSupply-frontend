...
> **Naming note — do not confuse this with the actual playbook.** Despite the similar name, this file is **not** the canonical [Engineering OS](https://github.com/TFRS-Admin/tfrs-engineering-playbook) (`tfrs-engineering-playbook`). That external repository, plus this repository's baseline files ([`AGENTS.md`](../AGENTS.md), [`CLAUDE.md`](../CLAUDE.md)), is the actual workflow authority for this repository. **Start at [`AGENTS.md`](../AGENTS.md), always** — not here.
>
> **Superseded as the constitution; retained as the canonical local historical design record.** As of the TFRS Engineering Playbook adoption sprint (see [`docs/engineering/archive/PLAYBOOK_ADOPTION.md`](./engineering/archive/PLAYBOOK_ADOPTION.md)), this file's proposed command vocabulary (`/spec /plan /build /test /review`), `PROJECT_STATE.md`/`IDEA_BACKLOG.md` file scheme, and self-declared "governing document" status below are **not** in effect — the adopted Engineering OS's lifecycle (`kernel/DECISION_ROUTER.md` → the matching `playbooks/*.md`, run live against `tfrs-engineering-playbook`) governs instead. Do not treat anything below this notice as authoritative for how work moves through this system.
>
> This file's own analysis is still useful context (source-of-truth hierarchy proposal, `docs/ai/*` and `docs/project-management/*` inventory), and it is now also the single retained record of everything unique from [`docs/ENGINEERING_OPERATING_SYSTEM.md`](./ENGINEERING_OPERATING_SYSTEM.md) — that file has been collapsed to a short pointer here, per this document's own Section 12. There is exactly one local historical design document from this point forward: this one. See [`docs/DOCUMENTATION_HIERARCHY.md`](./DOCUMENTATION_HIERARCHY.md) for the full documentation map.
