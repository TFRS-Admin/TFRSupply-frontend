...
- **Guided Vehicle Upfit Builder** (`/upfit-builder`, `GUIDED_UPFIT_BUILDER.md`): the wizard's first step ("Select Fleet Project") reads/writes `useFleetProject()` directly — creating or switching a project there is the same `createProject`/`setActiveProject` every other surface calls, no new project-selection logic.
