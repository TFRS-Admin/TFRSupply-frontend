...
The Shopify Storefront Catalog Adapter Foundation is documented in [SHOPIFY_STOREFRONT_CATALOG_ADAPTER.md](./SHOPIFY_STOREFRONT_CATALOG_ADAPTER.md). It is the first live-capable Storefront integration built on top of this foundation: its live adapter reuses `buildStorefrontFetchRequest()` unchanged and the existing `product-list-query`/`collection-query` operation types, but — unlike every adapter documented in this file — actually calls `fetch()` when given an explicit config (never sourced from build-time env). It introduces a separate `CatalogAdapter` boundary (distinct from `ShopifyStorefrontAdapter` above) that `catalogService` reads through with loader-based fallback, so `catalogService`'s public interface and default runtime behavior are unchanged.
