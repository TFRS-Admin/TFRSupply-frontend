...
The Review step's "Continue to Cart"/"Request a Quote" actions are joined by a "Generate Project Quote" button (`UpfitBuilderReviewStep`, gated by its own `showGenerateProjectQuote` prop) linking to `/project-quote` — the Fleet Quote Builder (`FLEET_QUOTE_BUILDER.md`). The button only appears once the active Fleet Project's overall quote readiness meets a configurable threshold: `GuidedUpfitBuilderPage.jsx` computes `buildFleetQuoteEntries(builds, activeProject, companyStandards)` and `resolveQuoteReadiness`/`meetsProjectQuoteReadinessThreshold` (`src/domain/fleetQuote/quoteReadiness.ts`) from the same project-scoped builds this page already reads — no new readiness logic is introduced here, and the threshold (`PROJECT_QUOTE_READY_LEVELS`) lives in one place, not duplicated per caller.
