...
## Fleet Procurement Packages integration

Fleet Procurement Packages (`FLEET_PROCUREMENT_PACKAGES.md`, `/procurement`) groups Fleet Builds by their **effective Department Standard** — the same `resolveEffectiveStandard` resolution this document defines — into named purchasing packages, and calls `summarizeFleetHealth` per package (instead of per project or account-wide) for each package's Vehicles/Completion figures. No new standard, tier, or completion rule is introduced; the only new logic is the grouping itself and a duplicate-vehicle-configuration check scoped to one package's builds.
