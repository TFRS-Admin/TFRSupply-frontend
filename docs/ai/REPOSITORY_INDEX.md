...
| `SHOPIFY_VARIANT_GID_OVERLAY.md` | Shopify Variant GID Overlay — `scripts/shopify-variant-gid-overlay/`, the only live Shopify API call in this repository: an Admin GraphQL client that pages through every Product Variant, a SKU matcher against the generated Variant Index, and validation that rejects malformed GIDs, duplicate SKU mappings, and mappings conflicting with a GID already on record; writes a `--gid-overlay`-ready file consumed unchanged by `SHOPIFY_CATALOG_CSV_INGESTION.md`'s existing `ingest.mjs`; reads credentials only from `SHOPIFY_STORE_DOMAIN`/`SHOPIFY_ADMIN_ACCESS_TOKEN` env vars and fails gracefully when they're missing; no storefront UI, checkout, or `commerceLookupService`/`shopifyVariantResolverService` contract changes |
