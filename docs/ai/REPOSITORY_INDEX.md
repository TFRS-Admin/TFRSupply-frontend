...
| `DESIGN_SYSTEM.md` | Design System & Layout Consolidation Foundation — presentational-only unification of every customer-facing page's layout/card/status vocabulary: reusable primitives (`src/components/design-system/`) for page shells (`PageLayout`/`PageHeader`/`PageSection`/`SectionToolbar`), cards (`DataPanel`/`StatCard`/`MetricCard`/`InfoCard`/`ActionCard`/`EmptyState`), layout (`SplitPanel`/`StickySidebar`/`PropertyGrid`/`ResponsiveTable`), and status (`StatusBadge`, backed by one `STATUS_TONE` map every readiness/completion/tier badge now resolves through); applied to Workspace/Project Quote/Procurement/Guided Upfit Builder/Product Detail/Homepage/Search/Compare/Saved Products' page shells and empty states with zero changes to routing, state, contexts, or `src/domain/*` logic |
