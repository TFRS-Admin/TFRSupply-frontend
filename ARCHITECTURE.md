...
- Two pre-existing test failures on `main` as of this writing (`tests/homepage-conversion-polish.test.mjs` → "uses mobile-first responsive grid classes for card sections", `tests/storefront-category-vertical-polish.test.mjs` → "renders every product in the category before any filter/search is applied") — see `docs/PLAYBOOK_ADOPTION.md` for verification evidence; not fixed as part of this adoption sprint (out of scope, tracked as known-red).
