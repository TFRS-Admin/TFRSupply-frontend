...
This repository's engineering process is governed by the Claude Code skills already installed on this account — `mattpocock-skills:*` ([`mattpocock/skills`](https://github.com/mattpocock/skills)) and `agent-skills:*` ([`addyosmani/agent-skills`](https://github.com/addyosmani/agent-skills)) — not by an external playbook repository. Both are registered as account-level plugin marketplaces in `~/.claude/settings.json` (`extraKnownMarketplaces`), tracking each maintainer's default branch rather than a pinned release as of 2026-09-13 — a maintainer decision on whether to pin is still open, see `.planning/findings.md` §0 in the audit branch that raised it. At the start of any non-trivial session, use `agent-skills:using-agent-skills` to route to the right skill (e.g. `agent-skills:spec-driven-development` / `planning-and-task-breakdown` for scoping work, `agent-skills:test-driven-development` / `mattpocock-skills:tdd` for implementation, `agent-skills:code-review-and-quality` / `mattpocock-skills:code-review` before a PR). This file adds project-specific rules on top of whatever skill is in play, and states a stricter local rule wherever one applies — this file wins on conflict with generic skill guidance.
...
This repository regularly has more than one agent session working it at once — the branch list carries ~130 branches, the large majority `claude/*` and `codex/*` agent-generated feature slices, evidence of ongoing parallel activity rather than a single serial actor. If you're picking up work here and another session might be active too, split by lane rather than by file, to keep concurrent sessions from stepping on each other's branches or docs:

- **Repo/infra lane** — branch protection, CI/workflow config (`.github/workflows/**`), dependency and security hardening, root governance docs (`AGENTS.md`, `CLAUDE.md`, `README.md`), secrets/DR runbooks, backlog process itself. Branch prefixes `infra/`, `docs/`. Avoid touching `src/**` feature code from this lane.
- **Feature/app lane** — product features, components, services, adapters (`src/**`), catalog/config data. Branch prefixes `feature/`, `bugfix/`, `refactor/`, `migration/`. Avoid touching `.github/workflows/**`, root governance docs, or repo/branch settings from this lane.

Both lanes still route every change through a PR against `main`, per Branch Pattern above — no lane merges its own PRs without the maintainer's explicit go-ahead, and no PR to `main` auto-merges (not via CI, a bot, or a scheduled routine) regardless of lane.
